...
#endif /* __cplusplus */
{
...
#else
    static const ::std::string base_command("./display.py");
#endif /* __cplusplus */
...
#if defined(__cplusplus)
    /* Inner delimiter (between coordinates). */
    static const ::std::string inner_delim(" ");
#endif /* __cplusplus */
    ...
    do
    {
        /* If the pointer `P` is a null-pointer, set the number `n` and `N` to
         * 0 and break the `do while`-loop. */
        if (!P)
        {
            /* Set the numbers `n` and `N` to 0. */
            n = 0U;
            N = 0U;
        ...
        command = (char*)malloc(
            (
                base_length +
                ((N * n * coordinate_length) << 1U) +
                (N - 1U) * delim_length + 1U
            ) * sizeof *command
        );
...
#if defined(__cplusplus)
        /* Set the output format to fixed and set the precision of the
         * output. */
        command << ::std::fixed;
        command << ::std::setprecision(static_cast<int>(prec));
#endif /* __cplusplus */

        /* Print the base command to the beginning of the command. */
#if !defined(__cplusplus)
        sprintf(command, base_command);
#else
...
#else
                command << inner_delim << *(P + ((i * n) << 1U) + j);
#endif /* __cplusplus */
        }
        ...
        /* Set the command string to an empty string. */
