 ...
 */
#if !defined(__cplusplus)
sign_t rsign (real_t x)
#elif (__cplusplus) < 201103L
inline sign_t rsign (real_t x)
#else
constexpr inline sign_t rsign (const real_t x)
#endif /* __cplusplus */
{
#if !defined(__cplusplus) || (__cplusplus) < 201103L
...
/**
 * Get the minimum of two real numbers.
 *
 * @param x
 *     Real number.
 *
 ...
 *
 * @return
 *     Minimum of {`x`, `y`}. If `x` or `y` is NaN, 0 is returned.
 *
 */
#if !defined(__cplusplus)
real_t rmin (real_t x, real_t y)
#elif (__cplusplus) < 201103L
inline real_t rmin (real_t x, real_t y)
#else
...
/**
 * Get the maximum of two real numbers.
 *
 * @param x
 *     Real number.
 *
 ...
 *
 * @return
 *     Maximum of {`x`, `y`}. If `x` or `y` is NaN, 0 is returned.
 *
 */
#if !defined(__cplusplus)
real_t rmax (real_t x, real_t y)
#elif (__cplusplus) < 201103L
inline real_t rmax (real_t x, real_t y)
#else
 ...
 * time of calling the function the function `rrand` is called an even number of
 * times (two values are generated; if the first value is 0 or 1, another two
 * values are generated; the process repeats until the first value is not 0 or
 * 1).  On each even time of calling the function no other function for
 * generating pseudorandom numbers is called.  As the Box-Muller transformation
 * generates two independent standanormally distributed values, one of the
 * values is returned immediately on each odd time of calling, while the other
 * is cached for the next call to the function.
 *
 * Caution: the function uses static variables to cache the generated values.
 * If more than one thread needs to call the function and it is likely that they
...
#endif /* __cplusplus */
{
    ...
    else
    {
         ...
         * function until the first value is not 0 or 1 and save them to `x` and
         * `y`. */
        do
        {
            x = rrand();
        ...
        while (rabs(x) < epsilon || rabs(1.0 - x) < epsilon);
...
{
#if !defined(__cplusplus)
    return (x && y) ? (int)rsign(*((const real_t*)x) - *((const real_t*)y)) : 0;
#else
    return (x && y) ?
        static_cast<int>(
            rsign(
