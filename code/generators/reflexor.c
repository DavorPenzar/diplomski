 ...
 * are not used since the resulting polygon can be generated using rectangular
 * rotations.
 *
...
int main (int argc, char** argv)
{
    ...
    P = (real_t*)malloc(((3U * n) << 1U) * sizeof *P);
    ...
    for (i = 0U; i < N; ++i)
    {
        ...
        /* Copy the coordinates of the polygon two times. */
        memcpy(P + (n << 1U), P, (n << 1U) * sizeof *P);
        memcpy(P + (n << 2U), P, (n << 1U) * sizeof *P);

        /* Generate reflexions of the polygon. */
        reflex_polygon(n, P + (n << 1U), true, false);
        reflex_polygon(n, P + (n << 2U), false, true);
