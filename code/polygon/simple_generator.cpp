 ...
 *
 * This is the C++ code.  The only purpose of this code is to test whether the
 * C++ code of the headers is correct or not.  The code itself actually uses
 * functions from the C language's standard library (e. g. `printf(...)` instead
 * of `::std::cout << ...`), except for memory allocation and deallocation
 * (operators `new[]` and `delete[]` are used instead of the `malloc(...)` and
 * `free(...)` functions).
 *
...
int display (::size_t n, const real_t* P)
{
    ...
    do
    {
        ...
        command = new char[base_length + 16U * n];
...
int main (int argc, char** argv)
{
     ...
     * argument. */
    n = static_cast< ::size_t >(::atoi(*(argv + 1U)));
